...
function [x, theta, history] = MMV_GSBL( J, F_fun, y, noise_var, R_fun, c, d, QUIET )
    ...
    for counter = 1:MAX_ITER
        ...
        % 1) x-updates 
        parfor j=1:J
            A_fun{j} = @(x) (1/noise_var{j})*FtF{j}(x) + R_fun( theta.*R_fun(real(x),'notransp'), 'transp'); % forward operator 
